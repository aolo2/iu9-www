...
module.exports.init = () => {
  mongodb.MongoClient.connect(config.mongo.url, (err, client) => {
    if (!err) {
      ...
      mongo_client.collection(usersCollectionName).ensureIndex('login', {unique: true}, (err) => {})
      mongo_client.collection(eventSubjectsCollectionName).ensureIndex('id', {unique: true}, (err) => {})
      mongo_client.collection(eventSubjectsCollectionName).ensureIndex('id',  {unique: true}, (err) => {})
      mongo_client.collection(eventTypesCollectionName).ensureIndex('id',  {unique: true}, (err) => {})
      mongo_client.collection(usersCollectionName).ensureIndex({'group': 1}, (err) => {})
      mongo_client.collection(usersCollectionName).ensureIndex({'roles': 1}, (err) => {})
      mongo_client.collection(newsCollectionName).ensureIndex({'public': 1}, (err) => {})
      mongo_client.collection(newsCollectionName).ensureIndex({'target': 1}, (err) => {})
      mongo_client.collection(editBoxCollectionName).ensureIndex({'box_id': 1}, (err) => {})
    }
  })
  return this
}
