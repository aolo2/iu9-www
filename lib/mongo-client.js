...
function get_subjects(callback) {
  do_client_action((client) =>
    client.collection(subjectsCollectionName)
    .find({}).toArray(callback), callback)
}

function add_subject(subject_name, callback) {
  do_client_action((client) =>
    client.collection(subjectsCollectionName).count((err, count) => {
      if (err) {
        callback(err)
      } else {
        client.collection(subjectsCollectionName)
        .insertOne({'name': subject_name, 'id': (count + 1).toString()}, callback)
      }
    }), callback)
}

function get_subject(subject_id, callback) {
  do_client_action((client) =>
    client.collection(subjectsCollectionName)
...
module.exports.get_subjects = get_subjects
module.exports.add_subject = add_subject
module.exports.get_subject = get_subject
// module.exports.
