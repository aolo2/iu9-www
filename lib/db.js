...
function get_news(request, callback) {
  let query = {'$or': [{'public': true}]}
  
  if (request.user_db) {
    query['$or'].push({'canSee': {'$in': request.user_db['roles']}})
  }
...
function update_article_source(article_id, source, html, callback) {
  mongo_client.update_article_source(article_id, source, html, callback)
}
...
module.exports.update_article_source = update_article_source
